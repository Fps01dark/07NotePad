...
CustomTextEdit::CustomTextEdit(std::shared_ptr<MessageBus> message_bus, QWidget* parent)
    : m_messageBus(message_bus), QTextEdit(parent) {
    InitUi();
    InitValue();
    InitConnect();
}
...
void CustomTextEdit::SetText(const QString& text) { setText(text); }

QString CustomTextEdit::GetText() const { return toPlainText(); }
