...
void MainCore::ExitSoftware() { m_messageBus->Publish("Exit Software"); }
...
void MainCore::InitValue()
{
	...
	m_messageBus->Subscribe("Exit Software", [=]() { m_mainWindow->close(); });
	...
	// Directory
	m_messageBus->Subscribe("Expand All", [=]() { m_dirWorkSpace->ExpandAll(); });
	m_messageBus->Subscribe("Collapse All", [=]() { m_dirWorkSpace->CollapseAll(); });
...
void MainCore::InitConnect() {
	...
	connect(m_centralWidget, &CustomTabWidget::tabCloseRequested,
		[=](int index) { m_messageBus->Publish("Close File", index); });
	connect(m_tabBar, &CustomTabBar::tabMoved,
		[=](int from, int to) { m_messageBus->Publish("Tab Moved", from, to); });
}
