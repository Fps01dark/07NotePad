...
void CustomTabBar::InitConnect() {
    ...
    connect(m_deleteAction, &QAction::triggered, [=]() { m_messageBus->Publish("Delete File"); });
    connect(m_reloadAction, &QAction::triggered, [=]() { m_messageBus->Publish("Reload File"); });
    connect(m_copyPathAction, &QAction::triggered, [=]() { m_messageBus->Publish("Copy Path"); });
    connect(m_copyNameAction, &QAction::triggered, [=]() { m_messageBus->Publish("Copy Name"); });
